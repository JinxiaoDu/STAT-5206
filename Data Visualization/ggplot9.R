...
library(ggplot2)
library(scales)
salary_boxplots<-ggplot(employee,aes(Year,Salaries))+
  geom_boxplot()+
...
## Do not modify this line!
salary_histograms<-ggplot(employee_trunc)+
  geom_histogram(aes(Salaries),binwidth=5000)+
  facet_wrap(~Year)+
  scale_y_continuous(label=comma)+
  scale_x_continuous(label=comma)+
  labs(x="Yearly salaries (USD)",y="",
       title = "Salaries have a slowly changing bimodal distribution")+
  theme_light()+
...
## Do not modify this line!
salary_densities<-ggplot(employee_trunc,aes(color=Year))+
  geom_density(aes(Salaries))+
  scale_x_continuous(label=comma)+
...
#    - finally add the light theme using `theme_light()`
## Do not modify this line!
log_salaries_qq<-ggplot(employee_culture,aes(sample=log10(Salaries)))+
  geom_qq()+
  geom_qq_line()+
