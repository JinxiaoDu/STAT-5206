...
fu1<-fu%>%
  ...
  filter(between(year,2001,2019),home_team %in% c("England","Germany","France","Belgium","Italy","Spain"))%>%
...
## Do not modify this line!
plot1<-ggplot(fu1)+
  geom_boxplot(aes(home_team,total,fill=home_team),alpha=0.6)+
...
## Do not modify this line!
library(ggridges)
plot2<-ggplot(fu2,aes(year,home_score,group=home_score,fill=home_score))+
  geom_density_ridges(aes(year,home_score),alpha= 0.6)+
...
#    # бн with 79 more rows
## Do not modify this line!
fu3<-fu%>%
  select(home_team,tournament,date)%>%
  mutate(tournament=factor(tournament,c('FIFA World Cup','UEFA Euro','UEFA Nations League', 'Nations Cup','Kirin Cup')))%>%
  drop_na()%>%
  filter(year(date)>2000,home_team %in% c('England','Germany','France','Belgium','Italy','Spain'),
         tournament %in% c('FIFA World Cup','UEFA Euro','UEFA Nations League', 'Nations Cup','Kirin Cup'))
...
## Do not modify this line!
plot3<-ggplot(fu3,aes(home_team,fill=tournament))+
...
## Do not modify this line!
plot4<-ggplot(fu4)+
  geom_bar(aes(team,stat(count)),fill="lightsalmon1",color="black",bin=10)+
  coord_flip()+
