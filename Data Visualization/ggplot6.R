...
g1<-ggplot(data=taxi,aes(x=fare_amount,y=tip_amount))+
...
g2<-ggplot(data=taxi,aes(x=fare_amount,y=tip_amount))+
...
taxi_new<-taxi %>%
  mutate(pickup_time=strftime(tpep_pickup_datetime,format='%H',tz='UTC')) %>%
...
g4<-ggplot(taxi_new,mapping=aes(x=pickup_time,y=tip_amount))+
  ...
  labs(title = "Min/Max/Median Tip Amount by Pickup Time",
       subtitle = paste("The highest tip was paid to a trip at 4AM.",
                        "9PM had the highest median tip whereas 8PM/10PM had the lowest.",sep="\n"),
       x = "Pickup Time (hour)",
       y = "Tip amount ($)")+
  theme_light()