...
#    # бн with 4,067 more rows, and 1 more variable: rings <dbl>
#'
...
## Do not modify this line!
g1<-ggplot(abalone)+stat_density(aes(x=rings,color=sex),geom="line",position="identity")+
  labs(title="Male and female abalone have similar modes in rings around 10,\nwhile infant abalone have lower mode in rings around 8",
       x="Number of rings",y="Density")+
...
## Do not modify this line!
g2<-ggplot(abalone)+
  geom_histogram(aes(diameter,fill=sex),binwidth = 0.05)+
  facet_grid(~sex)+
...
## Do not modify this line!
g3<-ggplot(abalone)+
  geom_boxplot(aes(x=reorder(sex,1:length(sex),order=TRUE),y=diameter))+
...
## Do not modify this line!
g4<-ggplot(abalone)+
  geom_point(aes(x=rings,y=diameter))+
  geom_smooth(aes(x=rings,y=diameter),method="loess")+
  facet_wrap(~sex)+
...
## Do not modify this line!
g5<-ggplot(abalone)+
  geom_violin(aes(x=length,y=whole_weight))+
  facet_wrap(~sex)+
