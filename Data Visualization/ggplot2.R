...
efficiency_by_manufacturer<-autompg %>%
  ...
  mutate(car_made=map_chr(car_name,~unlist(strsplit(.x," "))[1])) %>%
...
g1<-ggplot(efficiency_by_manufacturer)+
  geom_boxplot(aes(x=reorder(car_made,efficiency,median),y=efficiency))+
...
g2<-ggplot(efficiency_by_manufacturer)+
  ...
  labs(x="Weight of a car (kg)",y="Miles per gallon")+
...
## Do not modify this line!
g3<-g2+
  geom_smooth(aes(weight,mpg,color=model_year,linetype='r2'),
...
## Do not modify this line!
g4<-g3+
  geom_text_repel(data=best_efficiency_by_manufacturer,aes(weight,mpg,color=model_year,label=car_made), colour='black')+
  labs(title="Mpg is decreasing in weights",subtitle="How are cars performing in best efficiency by different car manufacturers?")+
