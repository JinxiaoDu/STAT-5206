...
## Do not modify this line!
age_plot<-ggplot(fifa)+
  geom_histogram(aes(Age,y=..density..),binwidth=2,color='white')+
  geom_density(aes(Age),color='blue')+
...
club_wage<-fifa%>%
  ...
  mutate(Club=fct_reorder(as.factor(Club),Wage,.desc=F))
...
## Do not modify this line!
wage_plot<-ggplot(club_wage)+
  geom_boxplot(aes(Club,Wage))+
...
#    2 200389 J. O¡­    25 http¡­ Slovenia    http¡­      90        93 Atl¨¦¡­ https://cd¡­ 6.80e7  94000
...
## Do not modify this line!
height_weight_plot<-ggplot(fifa_height_weight)+
  geom_point(aes(Height_m,Weight_kg,col=Position),alpha=0.8)+
...
## Do not modify this line!
agility_plot<-ggplot(agility)+
  geom_histogram(aes(Agility,stat(count)),bins=20,color='#FFFFFF')+
