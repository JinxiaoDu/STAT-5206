...
library(ggplot2)
library(scales)
tip_histogram<-ggplot(taxi)+
  geom_histogram(aes(tip_amount),binwidth = 1)+
...
#    name
## Do not modify this line!
tip_facet_hist<-ggplot(taxi_binned)+
  geom_histogram(aes(tip_amount),binwidth = 1)+
  scale_y_continuous(label=comma)+
...
#    `"Tip Amount (USD)"`.
## Do not modify this line!
hex_heatmap<-ggplot(taxi_binned)+
  geom_hex(aes(fare_amount,tip_amount),binwidth=c(2.5,0.5))+
