...
library(lubridate)
library(tidyverse)
wine<-drop_na(read_csv("data/winequalityN.csv"))
...
set.seed(0)
seed=.Random.seed
plot1_1<-ggplot(wine,aes(`fixed acidity`,quality))+
  geom_point()+
  labs(title = "The quality does not seem to be correlated with acidity",
       x = "Fixed Acidity",
       y = "Quality")

plot1_2<-ggplot(wine,aes(`fixed acidity`,quality))+
...
## Do not modify this line!
library(GGally)
plot3<-ggpairs(wine,aes(color=type),
               columns = c("density", "volatile acidity", "citric acid"),
...
## Do not modify this line!
plot4=ggplot(red,aes(`citric acid`,density))+
