...
## Do not modify this line!
t2<-billboard %>% 
  pivot_longer(cols=starts_with("wk"),names_to="week",names_prefix="wk",values_to="rank",values_drop_na=TRUE)
...
t2_rank<-t2 %>%
  ...
  mutate(last_rank=lag(rank)) %>%
  filter(week!=1) %>%
  summarize(highest_rank_increase=max(last_rank-rank))
