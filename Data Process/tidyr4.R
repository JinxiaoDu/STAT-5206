...
stats_by_state<-mobility %>%
  group_by(State) %>%
  summarize_at(c("Commute","Mobility"),funs(mean,sd))
...
#    # бн with 194 more rows
## Do not modify this line!
stats_by_state_longer<-stats_by_state %>%
  pivot_longer(-State,names_to="wxl",values_to="Value") %>%
  separate(wxl,into=c("Variable","Stat"))
...
coordinates<-mobility %>%
  group_by(State) %>%
  summarize(Latitude_mean=round(mean(Latitude),digits=3),Longitude_mean=round(mean(Longitude),digits=2)) %>%
