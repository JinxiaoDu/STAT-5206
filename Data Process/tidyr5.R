...
summary_stats<-iris %>%
  select(-Species) %>%
  summarise_if(is.numeric,list(Min.=min,"1st Qu."=quantile1,Median=median,Mean=mean,'3rd Qu.'=quantile3,Max.=max))
summary_stats
...
summary_long<-summary_stats %>%
  pivot_longer(names_to="key",values_to="value",everything()) %>%
...
summary_stats_transposed<-summary_stats_tidy %>%
  pivot_longer(-variable,names_to="Stat",values_to="wxl") %>%
  pivot_wider(names_from=variable,values_from=wxl)
