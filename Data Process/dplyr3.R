...
airquality_replace_na$Ozone[is.na(airquality_replace_na$Ozone)]=mean(airquality_replace_na$Ozone,na.rm=TRUE)
...
airquality_wind<-airquality_filtered %>%
  mutate(Wind=case_when(Wind>=1&Wind<9~7.4,Wind>=9&Wind<=20~11.5,TRUE~21))
