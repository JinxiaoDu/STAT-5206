...
mean_max_min<-abalone %>%
  group_by(sex) %>%
  summarize(weight_mean=mean(shucked_weight),weight_max=max(shucked_weight),weight_min=min(shucked_weight))
...
#    # бн with 2,953 more rows, and 2 more variables: viscera_weight <dbl>,
#     shell_weight <dbl>
## Do not modify this line!
...
#    509.              15.5
#    894.             109.
#    # бн with 4,167 more rows
## Do not modify this line!
transmute<-abalone %>%
  transmute(whole_weight_in_mg=1000*whole_weight,water_weight_in_mg=1000*(whole_weight-shucked_weight-viscera_weight-shell_weight))
