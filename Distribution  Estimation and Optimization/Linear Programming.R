...
x<-as.data.frame(x)
g1<-ggplot(x,aes(x=x)) +
  stat_function(fun=function(x) (40*60-50*x)/24) +
  annotate("text",x=35,y=20,label="Constraint of Machine A") +
  stat_function(fun=function(x) (35*60-30*x)/33) +
...
## Do not modify this line!
g2<-g1+
  stat_function(fun=function(x) -x+56)+
  stat_function(fun=function(x) -x+51.25)+
  stat_function(fun=function(x) -x+50.6)
x1<-45
y1<-6.25
...
## Do not modify this line!
library(lpSolve)
wxl<-matrix(c(-50,-24,-30,-33,1,0,0,1),nrow=4,byrow=TRUE)
s1<-lp(directio="max",objective.in=c(1,1),const.mat=wxl,const.dir=c(">=",">=",">=",">="),const.rhs=c(-40*60,-35*60,45,5))
...
#
## Do not modify this line!
wxl<-matrix(c(1.2,1.3,0.7,0,0.5,0.7,2.2,1.6,0.5,1,0.9,0.7,1.3,1,0.8,1.4,2.8,0.5,1.2,0.6),
            nrow=4,byrow=TRUE)
s2<-lp(direction="max",objective.in=c(18,25,10,12,15),const.mat=wxl,const.dir=c("<=","<=","<=","<="),
       const.rhs=c(4*40,5*40,3*40,7*40),all.int=TRUE)
