...
#    4 x 4 square.
## Do not modify this line!
himmelblau_plot<-wireframe(fnxy ~ x * y,data=xy,shade=TRUE,scales=list(arrows = FALSE),screen=list(z=-300, x=-70, y=0),
...
softline_dir<-crossing(x=c(1,-1),y=c(1,-1)) %>%
  ...
  mutate(a=map_dbl(d0,~softline(x0,.x,himmelblau))) %>%
  mutate(optimum=map2(d0,a,wxl)) %>%
...
pts<-tribble(~optimum,~method,
             ...
             optim_minima,"optim") %>%
  bind_rows(softline_dir %>%dplyr::select(optimum,method)) %>%
...
compare_plot<-contour+
  geom_point(data=pts,aes(x=x,y=y,color=method),size=2)+
