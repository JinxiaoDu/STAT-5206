...
mom_lnorm<-function(par){
  expectation<-exp(par[1]+0.5*par[2]^2)
  variance<-exp(2*par[1]+par[2]^2)*(exp(par[2]^2)-1)
...
obj_lnorm_factory<-function(x){
  function(par){
    Ex<-mean(x)
    Varx<-var(x)
    mom_E<-mom_lnorm(par)[1]
    mom_V<-mom_lnorm(par)[2]
    wxl<-(Ex-mom_E)^2+(Varx-mom_V)^2
    return(wxl)
  }
}
...
par_lnorm<-function(x){
  wxl<-optim(c(1,1),obj_lnorm_factory(x),hessian=TRUE)
  se<-sqrt(diag(solve(wxl$hessian)))
  frozen<-tibble(parameter=c('mu_mom','sd_mom'),
                 value=wxl$par,se=se,lower=wxl$par-1.96*se,upper=wxl$par+1.96*se)
  return(frozen)
}
...
nll_lnorm(c(1,1),x)
lzl<-optim(c(1,1),partial(nll_lnorm,x=x))$par
mu_mle<-lzl[1]
sd_mle<-lzl[2]
...
library(ggplot2)
x=as.data.frame(x)
compare_plot<-ggplot(x,aes(x = x))+
  geom_histogram(aes(y=..density..),bins=50)+
  geom_density(aes(color="Data"))+
  stat_function(fun=partial(dlnorm,meanlog=mu_mom,sdlog=sd_mom),aes(color="MOM"))+
  stat_function(fun=partial(dlnorm,meanlog=mu_mle,sdlog=sd_mle),aes(color="MLE"))+
  labs(title = "Both methods seem to do a good job",
       x = "Simulated x",y = "Density")+
  theme_light()+
