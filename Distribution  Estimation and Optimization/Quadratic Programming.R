...
stocks_data_longer<-stocks_data %>%
  pivot_longer(c(2:11),names_to="stock",values_to="return")
...
## Do not modify this line!
stock_return_plot<-ggplot(stocks_data_longer,aes(x=date,y=return,group=stock))+
...
mu=c(0)
for(i in c(2:11)){
  mu[i-1]=mean(unlist(stocks_data[,i]),na.rm=TRUE)
}
Amat<-rbind(rep(1, 10),mu,diag(10),-diag(10))
...
## Do not modify this line!
portfolio_plot<-ggplot(portfolio,aes(x=stock,y=proportion))+
  geom_bar(stat='identity',fill='blue')+
  labs(title='Portfolio',subtitle="WMT has the highest proportion",
       x="",y="Proprtion")+
  coord_flip()+
  theme_light()
