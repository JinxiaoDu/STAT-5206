...
find_cost<-function(X, y, theta){
  m<-length(y)
  return(1/m*(sum(-y*log(sigmoid(X%*%theta))-(1-y)*log(1-sigmoid(X%*%theta)))))
}
...
broom::tidy(logit)
estimates<-broom::tidy(logit)%>%dplyr::select(term, estimate)%>%cbind(estimate_newton)
