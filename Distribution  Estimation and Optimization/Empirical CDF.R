...
#    5 Fr??d??ric C¡­   2CXfRXB¡­ Scherzo N¡­       0.992         0.318
...
spotify<-spotify %>%
  mutate(popularity_label=cut(popularity,breaks=c(0, 30, 70, 100),right=TRUE)) %>%
  mutate(popularity_label=fct_recode(popularity_label,unpopular="(0,30]",fair="(30,70]",popular="(70,100]")) %>%
  drop_na()
...
danceability_dist<-ggplot(spotify)+
  geom_histogram(aes(x = danceability, y = ..density..),binwidth=0.04,color='black',fill='orange')+
...
qq_plot_tempo<-ggplot(spotify_bts,aes(sample = tempo))+
  ...
  labs(title="Normal distribution fit for BTS tempo",
       ...
       x="normal distributed value",y="sample value")+
  theme_light()
...
nll_gamma_factory <- function(x){
  function(par){
    ...
    sx=sum(x)
    slx=sum(log(x))
    wxl=-(a-1) * slx + n * lgamma(a) + n * a * log(s) + sx / s
    return(wxl)
  }
}
...
#    artist_name track_id track_name acousticness danceability
#    <chr>       <chr>    <chr>             <dbl>        <dbl>
#    1 Z¨¦ Neto & ¡­ 1fyhyOy¡­ Status Qu¡­       0.59          0.580
...
## Do not modify this line!
liveness_gamma_plot<-ggplot(spotify_pop,aes(x=liveness))+
  geom_histogram(aes(x = liveness, y = ..density..),binwidth=0.02,color='black',fill='orange')+
  geom_density(kernel="gaussian",bw=0.06,color='red',fill='red',alpha=0.5)+
...
qq_plot_liveness<-ggplot(spotify_pop,aes(sample=liveness))+
  ...
  labs(title="Gamma distribution fit for song liveness",
       ...
       x="gamma distributed value",y="sample value")+
  theme_light()
