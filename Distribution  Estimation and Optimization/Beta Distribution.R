...
x_ba=mean(x)
v_ba=var(x)
alpha0=x_ba*(x_ba*(1-x_ba)/v_ba-1)
beta0=(1-x_ba)*(x_ba*(1-x_ba)/v_ba-1)
...
obj_beta_factory<-function(x){
  force(x)
  function(par){
    return(sum((mean(x)-mom_beta(par)[1])^2+(var(x)-mom_beta(par)[2])^2))
  }
}
...
wxl<-optim(c(1,1),nll_beta,x=x,hessian=T,lower=0,method="L-BFGS-B")
alpha2<-wxl$par[1]
beta2<-wxl$par[2]
sd<-sqrt(diag(solve(wxl$hessian)))
lower=wxl$par-sd*1.96
upper=wxl$par+sd*1.96
...
g1<-ggplot(x,aes(x=x))+
  ...
  stat_function(fun=ycl,aes(color='MOM'))+
  stat_function(fun=lrh,aes(color='MLE'))+
  stat_function(fun=wxl,aes(color='True Value'))+
