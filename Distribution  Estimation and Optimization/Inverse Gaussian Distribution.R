...
obj_invgauss<-function(par,x){
  ...
  sum((moments-mom_invgauss(par))^2)
}
...
g1<-ggplot(random_data_tbl,aes(sample=random_data))+
  ...
  labs(title="The sample seems to distribute along the straight line",
       ...
       x="Theoretical values",y="Actual values")+
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
nll_invgauss<-function(par, x){
  return(-sum(dinvgauss(x,par[1],par[2],log=TRUE)))
}
set.seed(100)
seed2<-.Random.seed
...
shape_hat<-mu_hat^3/var(random_data)
...
g2<-ggplot(random_data_tbl,aes(x = random_data))+
  ...
  stat_function(fun=partial(dinvgauss,mean=par1[1],shape=par1[2]),aes(color="MOM")) +
  stat_function(fun=partial(dinvgauss,mean=par2[1],shape=par2[2]),aes(color="MLE")) +
  stat_function(fun=partial(dinvgauss,mean=2,shape=3),aes(color="True Value")) +
