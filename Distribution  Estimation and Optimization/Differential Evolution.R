...
set.seed(100)
seed3<-.Random.seed
de_bowl<-DEoptim(obj_bowl,c(-4,-4),c(4,4))
set.seed(100)
seed4<-.Random.seed
de_ackley<-DEoptim(obj_ackley,c(-4,-4),c(4,4))
...
## Do not modify this line!
library(ggplot2)
g1<-ggplot(value_of_params,aes(x=x,y=y,color=objective))+
  geom_line()+
...
value_of_params_pivot<-value_of_params %>%
  pivot_longer(c(1,2,4),names_to="name",values_to="value")
g2<-ggplot(value_of_params_pivot,aes(x=iteration,y=value,color=name))+
  geom_line()+
  scale_x_log10()+
  ...
  labs(title="Evolution of parameters on 200 iterations",
       ...
       x="Iterations",y="Value of parameters",color="Name")+
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust = 0.5))
