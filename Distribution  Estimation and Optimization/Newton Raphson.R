...
library(dplyr)
library(ggplot2)
x<-tibble(jay=seq(-30,30,0.01)) %>%
  mutate(jolin=f(jay))
f_plot<-ggplot(x,aes(x=jay,y=jolin))+
  geom_line()+
...
multi_step<-function(f,x,h,n_iterations)
{
  ...
  return(enframe(m) %>% rename(step=name,x=value))
}
multi_step(f,1,h,2)
...
newton_steps_tibble<-tibble(start=seq(-7, 7, 2)) %>%
  mutate(step=map(start,~multi_step(f,.x,h,n_iterations = 2))) %>%
  unnest(step) %>%
  mutate(f=f(x),start=factor(start),step=factor(step)) 
...
#    - add `theme_light()`
## Do not modify this line!
newton_steps_plot<-ggplot(newton_steps_tibble,aes(x=x,y=f))+
  stat_function(fun=f)+
  geom_point(aes(color=step),size=2)+
  geom_hline(yintercept=0,alpha=0.2)+
  facet_wrap(~start)+
  labs(title = "The points get closer to roots of f as steps increase",y = "f(x)")+
  theme_light()
...
#
## Do not modify this line!
plot_with_root<-f_plot+geom_point(x=root,y=f(root),colour="red") +
