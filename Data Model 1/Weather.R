...
#    # бн with 722 more rows
#'
## Do not modify this line!
weather_arrange<-weather_9am %>%
  full_join(weather_3pm,by=c('Date','WindDir','WindSpeed','Humidity','Pressure','Cloud','Temp')) %>%
...
## Do not modify this line!
temp_humidity_plot<-ggplot(weather_arrange,mapping=aes(x=`Day of the year`))+
...
weather_wind<-weather_wind_9am %>%
  full_join(weather_wind_3pm,by=c("WindDir","Temp")) %>%
  drop_na() %>%
