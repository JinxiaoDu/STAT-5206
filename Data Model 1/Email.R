...
email_id_to_name<-function(email_id){
  return(map_chr(str_split(email_id,"\\."),~str_to_title(str_trim(paste0(.x,collapse=' ')))))
}
...
g1<-ggplot(t2,aes(x=hour))+
  ...
  labs(title = "People send more emails during noon",x="Hour",y="Count(n)")+
  theme_light()+
...
g2<-ggplot(p2,aes(x=date,y=count,color=sender_role))+
  ...
  labs(title = "People are using emails more frequently in 2001 than 1999",
       x = "Date",y="Count(n)",color="Sender Role")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5))
