...
#    # бн with 49,990 more rows
## Do not modify this line!
ratings$date=with_tz(as.POSIXct(ratings$timestamp,origin = "1970-01-01"),tzone = "UTC")
...
#        the x-axis as `"Year"`,
#        the y-axis as `"Average ratings"`.
#    Store the plot into a variable `g1`.
## Do not modify this line!
g1<-ggplot(average_rating_in_different_years,aes(x=year_of_rating,y=average_rating_of_year))+
  geom_point()+
  labs(title="2004 and 2005 have lower average ratings",subtitle="While 2014 has highest average ratings",
...
#    <dbl> <chr>                           <chr>                                   <dbl>
#    1       1 Toy Story (1995)                Adventure|Animation|Children|Comedy|Faбн  1995
...
#    5       5 Father of the Bride Part II (1бн Comedy                                   1995
...
## Do not modify this line!
movies<-movies %>%
  mutate(year=as.numeric(gsub(title,pattern = ".+\\(([0-9]+)\\)",replacement = "\\1")))
...
## Do not modify this line!
g2<-ggplot(ratings_of_movies,aes(x=genres,y=rating))+
  geom_boxplot() +
...
## Do not modify this line!
g3<-ggplot(average_rating_of_genres,aes(x=number_of_ratings,y=average_rating))+
  ...
  labs(title="When number of ratings is over 5000, the average ratings start to be constant around 3.5",
       x="Number of ratings",y="Average rating",color="Genres")+
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
#    1 Toy Story (199бн Adventure  1995    3.5 2005-03-17 20:22:12 3728.8488 daбн        10.2
#    2 Toy Story (199бн Adventure  1995    3   1999-07-06 18:41:32 1647.7788 daбн         4.51
#    3 Toy Story (199бн Adventure  1995    3   2009-08-18 18:31:29 5343.7719 daбн        14.6
#    4 Toy Story (199бн Adventure  1995    4.5 2009-08-20 06:36:02 5345.2750 daбн        14.6
#    5 Toy Story (199бн Adventure  1995    5   1997-01-26 14:23:23  756.5996 daбн         2.07
#    6 Toy Story (199бн Adventure  1995    5   2001-01-18 17:22:03 2209.7236 daбн         6.05
#    7 Toy Story (199бн Adventure  1995    3.5 2004-03-06 07:59:24 3352.3329 daбн         9.18
#    8 Toy Story (199бн Adventure  1995    4   1996-10-24 22:12:42  662.9255 daбн         1.82
#    9 Toy Story (199бн Adventure  1995    4   1997-11-20 12:59:43 1054.5415 daбн         2.89
#    10 Toy Story (199бн Adventure  1995    4   2005-01-27 20:49:25 3679.8677 daбн        10.1
#    # бн with 130,077 more rows
## Do not modify this line!
ratings_over_time<-ratings_of_movies %>%
  mutate(wxl=make_datetime(year,1,1,00,00,00)) %>%
  mutate(interval=date-wxl)%>%
  #mutate(years_passed=as.double(dyears(as.double(interval)/365))/31536000) %>%
  mutate(years_passed=time_length(date-wxl,'year')) %>%
