...
#    # бн with 337 more rows
## Do not modify this line!
room_earning<-booking %>%
  left_join(rate,c("occupants"="occupancy","room_type_requested"="room_type")) %>%
...
#    # бн with 66 more rows
## Do not modify this line!
room_earning_filtered<-room_earning %>%
  filter(room_no<200,booking_date>='2016-11-15',booking_date<='2019-12-15') %>%
...
## Do not modify this line!
room_earning_plot<-ggplot(room_earning_filtered,mapping=aes(x=booking_date,y=earning))+
  geom_line()+
  scale_x_date(breaks = as.Date(c("2016-11-15", "2016-12-01", "2016-12-15")), date_labels = "%m/%d")+
  labs(title="Most rooms have earning around 100 to 300",x="Booking date",y="Earning (dollars)")+
  facet_wrap(~room_no)+
  theme_light()
room_earning_plot
...
guest_spending<-guest %>%
  ...
  right_join(room_earning,c("id"="guest_id")) %>%
  group_by(name) %>%
...
## Do not modify this line!
guest_spending_plot<-ggplot(guest_spending,aes(y=spending,x=name))+
  geom_col()+
  coord_flip()+
