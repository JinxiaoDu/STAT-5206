...
#        # бн with 2.161e+04 more rows, and 13 more variables: view <int>,
...
data_clean<-data %>%
  ...
  mutate(waterfront=fct_recode(waterfront,No="0",Yes="1")) %>%
  mutate(quarter=factor(quarter(date))) %>%
  mutate(age=2019-yr_built) %>%
  mutate(renovated=factor(yr_renovated!=0,labels=c("No","Yes"))) %>%
...
#    side-by-side.
## Do not modify this line!
g1<-ggplot(data_clean,mapping=aes(x=waterfront,y=price))+
  geom_boxplot()+
  coord_flip()+
  labs(title = "House Price Increases With A View To Waterfront",
       x = "View to waterfront",y = "Price ($)")+
  theme_light()
g2<-ggplot(data_clean,mapping=aes(x=renovated,y=price))+
  geom_boxplot()+
  coord_flip()+
  labs(title = "House Price Increases With A Renovation",x = "Renovation",y = "Price ($)")+
  theme_light()
...
## Do not modify this line!
g3<-ggpairs(data_clean,columns=c('price','sqft_lot','sqft_above','sqft_living','sqft_basement'))+
...
#        4    0.147   5.63
#        5    0.0657  5.64
#        # бн with 2.161e+04 more rows
## Do not modify this line!
data_model<-data_clean %>%
  dplyr::select(-quarter,-zipcode,-sqft_above,-sqft_basement,-sqft_living15,
                -sqft_lot15,-sqft_lot)
...
#        # бн with 2.036e+04 more rows, and 6 more variables: condition <int>,
...
#         # бн with 4 more variables: AIC <dbl>, BIC <dbl>, deviance <dbl>,
#         #   df.residual <int>
...
g6<-ggplot(fit_m2,aes(x=preds,y=resids))+
  geom_point(alpha=0.3)+
  geom_hline(yintercept=0,color='red')+
  labs(title = "Residuals Have No Pattern",subtitle = "No outliers are found.",
       x = "Fitted Prices ($)",y = "Residuals ($)")+
  theme_light()
g7<-ggplot(fit_m2,aes(sample=resids))+
  stat_qq()+
  stat_qq_line()+
...
by_quarter<-data_clean %>%
  dplyr::select(-zipcode,-sqft_above,-sqft_basement,-sqft_living15,-sqft_lot15,-sqft_lot) %>%
...
#     1 1          [3,836 б┴ 13] <lm>   <tibble [3,836 б┴ бн <tibble [3,836 б┴ бн
#     2 2          [6,427 б┴ 13] <lm>   <tibble [6,427 б┴ бн <tibble [6,427 б┴ бн
#     3 3          [5,612 б┴ 13] <lm>   <tibble [5,612 б┴ бн <tibble [5,612 б┴ бн
#     4 4          [4,489 б┴ 13] <lm>   <tibble [4,489 б┴ бн <tibble [4,489 б┴ бн
## Do not modify this line!
f<-function(df){
...
by_quarter_fit<-by_quarter_resid %>%
  inner_join(by_quarter_pred,keys=c("quarter","key"))
...
## Do not modify this line!
g8<-ggplot(by_quarter_fit,aes(x=pred,y=resid))+
  geom_point(alpha = 0.3)+
  geom_hline(yintercept = 0,color='red')+
  facet_wrap(~quarter)+
