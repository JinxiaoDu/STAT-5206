...
temperature_change<-temperature %>%
  filter(year %in% c(1910,2010) & month=='Mar') %>%
  group_by(country) %>%
  arrange(year) %>%
  summarize(temperature=temperature[2]-temperature[1])
...
temperature_map<-ggplot(temperature_change,aes(map_id=country))+
  ...
  expand_limits(x=-180:180,y=-90:90)+
...
## Do not modify this line!
switzerland_temperature_plot<-ggplot(sw,aes(x=month,y=temperature))+
  geom_boxplot()+
...
## Do not modify this line!
sw_mod<-sw %>%
  lm(formula=temperature~month)
names(sw_mod[[1]])<-c("(Intercept)","monthFeb","monthMar","monthApr",'monthMay',"monthJun",
...
sw_seasonal_plot<-switzerland_temperature_plot+
  geom_point(data=sw %>% filter(temperature==mean(temperature)),color="green",size=2,pch=3)+
...
## Do not modify this line!
residuals_plot<-ggplot(sw_residuals,aes(x=year,y=resid))+
  geom_point()+
...
## Do not modify this line!
sw_improved_mod<-sw%>%
  lm(formula=temperature~year+month)
names(sw_improved_mod[[1]])<-c("(Intercept)","year","monthFeb","monthMar","monthApr",'monthMay',
...
country_model<-function(df){
  lm(formula=temperature~year+month,data=df)
}

by_country<-temperature %>%
  group_by(country,region) %>%
  nest() %>%
  mutate(model=purrr::map(data,~country_model(.x)))%>%
  mutate(residuals=map2(data,model,~add_residuals(.x,.y))) %>%
  mutate(predictions=map2(data,model,~add_predictions(.x,.y)))
...
## Do not modify this line!
monthly_pattern_plot<-ggplot(monthly_pattern,aes(x=month,y=pred,color=country,group=country))+
  geom_point()+
  geom_line()+
...
worst_predict <- by_country %>% 
  mutate(glance = purrr::map(model, ~broom::glance(.x))) %>% 
  unnest(glance) %>%
