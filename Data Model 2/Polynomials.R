...
quad_resid_plot<-ggplot(data_with_quad_resid,aes(x=x,y=resid))+
  geom_ref_line(h=0)+
  geom_point()+
  labs(y="Residuals",title="The residuals show a secondary pattern that is not captured")
...
#     # бн with 1,990 more rows
## Do not modify this line!
library(purrr)
library(tidyr)
my_model<-function(k){
  train_data %>%
    lm(formula=y~poly(x,k,raw=TRUE))
}

model_pred_resid<-tibble(k=1:10)%>%
  mutate(model=map(k,my_model)) %>%
  mutate(data=map(model,~add_residuals(add_predictions(all_data,.x),.x))) %>%
  unnest(data) %>%
  select(-model)
...
## Do not modify this line!
wxl<-model_pred_resid %>%
  filter(k==best_deg)
residual_plot<-ggplot(wxl,aes(x=x,y=resid))+
  geom_point()+
  geom_hline(yintercept=0,size=1)+
  labs(y = "Residuals",title = "No clear pattern in the residuals",
       subtitle = "The model did its job!")
...
## Do not modify this line!
ycl<-model_pred_resid %>%
  filter(k==7|k==best_deg) %>%
  mutate(k=factor(k))
prediction_plot<-ggplot(ycl,aes(x=x,y=y))+
  geom_point()+
  geom_line(aes(x=x,y=pred,color=k),size=1)+
  labs(color = "Polynomial Degree",title = "Both models fit the data quite well",
       subtitle = "Except close to the boundaries, is the model with k = 9 really better?")
...
model_mse_penalized<-model_pred_resid %>%
  group_by(k,split) %>%
  nest() %>%
  mutate(errors=map(data,~my_mse_penalized(.x,k,lambda=c(0, 0.1, 0.4))))%>%
  unnest(errors)%>%
  select(-data)
...
## Do not modify this line!
lrh<-model_mse_penalized %>%
  mutate(lambda=factor(lambda))
mse_penalized_plot<-ggplot(lrh,aes(x=k,y=loss,color=split,linetype=lambda))+
  geom_line()+
  labs(color = "Split",x = "Polynomial Degree",y = "Loss",linetype = expression(lambda),
       title = "A stronger penalization implies a smaller optimal degree",
       subtitle = expression(paste(lambda," = 0 corresponds to the unpenalized case")))
