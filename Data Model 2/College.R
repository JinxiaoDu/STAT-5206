...
#    # бн with 767 more rows, and 10 more variables: Outstate <dbl>,
...
#    9      32439 Yes            30    79
#    10      25299 Yes            21    40
#    # бн with 767 more rows
## Do not modify this line!
df<-college %>%
  rename_all(str_replace_all,'\\.','_') %>%
  mutate(total_cost=Outstate+Room_Board+Books+Personal+Expend,Private=factor(Private)) %>%
...
## Do not modify this line!
g1<-ggplot(df,aes(x=Top10perc,y=total_cost))+
  geom_point()+
...
## Do not modify this line!
m0<-df %>%
  lm(formula=total_cost~Top10perc)
total_cost_pred<-add_predictions(add_residuals(df,m0),m0) %>%
...
## Do not modify this line!
g2<-ggplot(total_cost_pred,aes(x=pred0,y=resid0))+
  geom_ref_line(h=0)+
  geom_point()+
  labs(title="The residuals have a fanning pattern that spread to the right",
       ...
       x="Prediction of total cost (USD)",y="Residuals of m0")+
  my_theme
...
## Do not modify this line!
m1<-total_cost_pred %>%
  lm(formula=log_total_cost~Top10perc)
total_cost_pred<-add_predictions(add_residuals(total_cost_pred,m1),m1) %>%
...
## Do not modify this line!
g3<-ggplot(total_cost_pred)+
  geom_point(aes(x=Top10perc,y=log_total_cost))+
  geom_line(aes(y=pred1,x=Top10perc),color='red')+
  labs(title="After taking log of total cost, the log of total cost has a
       more linear relationship with the students from top 10% H.S.",
       x="Pct. of new students from top 10% H.S.",y="Log of total cost (log (USD))")+
  my_theme
...
g4<-ggplot(total_cost_pred)+
  ...
  labs(title="The residuals don't have obvious pattern",
       ...
       x="Pct. of new students from top 10% H.S.",y="Residuals of m1")+
  my_theme
...
## Do not modify this line!
m2<-total_cost_pred%>%
  lm(formula=log_total_cost~Private*Top10perc)
total_cost_pred<-add_predictions(add_residuals(total_cost_pred,m2),m2) %>%
...
## Do not modify this line!
g6<-ggplot(total_cost_pred)+
  geom_ref_line(h=0)+
  geom_point(aes(x=pred2,y=resid2))+
...
#         `"`
#         `the total cost"`
#         the x-axis as `"Pct.Whether a college is private or not can help to differentiate of new students from top 10% H.S."`,
#         the y-axis as `"Log of total cost (log (USD))"`.
#     Store the plot into a variable `g7`
## Do not modify this line!
g7<-ggplot(stats_m2,aes(x=Top10perc,y=log_total_cost))+
  geom_point()+
  facet_grid(~Private)+
  geom_smooth(aes(linetype=Type),se=FALSE)+
  labs(title="The m2 model seems to fit well on both public colleges and private colleges",x="Pct. of new students from top 10% H.S.",
...
## Do not modify this line!
g8<-ggplot(total_cost_pred,aes(x=PhD^2,y=resid2))+
  geom_point()+
  geom_smooth(method='lm',se=FALSE)+
  labs(title="The residuals of m2 seem to have
       ...
       x="Square of pct. of faculty with PhD",y="Residuals of m2")+
  my_theme
...
#     10  0.108    10.0
## Do not modify this line!
m3<-lm(formula=log_total_cost~Private*Top10perc+poly(PhD,2),data=total_cost_pred)
m3 %>% broom::tidy()
total_cost_pred<-add_predictions(add_residuals(total_cost_pred,m3),m3) %>%
...
g9<-ggplot(total_cost_pred,aes(x=pred3,y=resid3))+
  ...
  labs(title="There's no obvious pattern of in the residuals",
       ...
       x="Prediction of log of total cost (log (USD))",y="Residuals of m3")+
  my_theme
...
## Do not modify this line!
g10<-ggplot(r_squared_and_adjusted)+
