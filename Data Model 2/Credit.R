...
#    20.1  2525    200     3    57        15 Female No      Yes     African бн
#    53.6  3714    286     3    73        17 Female No      Yes     African бн
#    20.1  2631    213     3    61        10 Male   No      Yes     African бн
...
#    бн with 390 more rows, and 2 more variables: Balance <dbl>,
#    Limit_per_card <dbl>
#
...
## Do not modify this line!
g2<-ggplot(credit_new,aes(x=Limit_per_card,y=Balance,color=Student))+
  geom_point()+
  geom_smooth()+
  labs(title="The balance goes higher as per card limit goes up",
       subtitle = "This relationship is nonlinear",
       x = "Limit per card (USD)",y = "Balance (USD)")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
credit_new<-credit_new %>%
  ...
  filter(Balance>0)
g3<-ggplot(credit_new,aes(x=log_limit_per_card,y=Balance,color=Student))+
  geom_point()+
  geom_smooth()+
  labs(title = "The balance goes higher as log of per card limit goes up",
       subtitle = "This relationship looks more linear after transformation",
       x = "Log of Limit per card (USD)",y = "Balance (USD)")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
#    Income Limit Rating Cards   Age Education Gender Student Married Ethnicity
#    <dbl> <dbl>  <dbl> <dbl> <dbl>     <dbl> <fct>  <chr>   <fct>   <fct>
#    16.3  1160    126     3    78        13 Male   Yes     Yes     African бн
#    44.5  3500    257     3    81        16 Female No      No      African бн
#    88.8  4952    360     4    86        16 Female No      Yes     Caucasian
#    20.9  1233    128     3    47        18 Female Yes     Yes     Asian
#    33.0  3180    224     2    28        16 Male   No      Yes     African бн
#    34.5  3271    250     3    57        17 Female No      Yes     Asian
#    35.0  3327    253     3    54        14 Female No      No      African бн
...
g32<-ggplot(wxl,aes(x=log_limit_per_card,y=Balance,color=Student))+
  geom_point(data=credit_pred,aes(log_limit_per_card,Balance))+
  geom_smooth(mapping=aes(linetype=Type),se=F)+
...
## Do not modify this line!
g4<-ggplot(credit_pred,aes(y=resid1,x=Balance))+
  geom_point()+
  geom_ref_line(h=0,size=0)+
  labs(title="The residuals goes up as balance goes up",
       subtitle = "The range of residuals is pretty wide",
       x = "Balance (USD)",y = "Residuals (USD)")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
## Do not modify this line!
g5<-ggplot(wxl)+
  geom_smooth(aes(y=Balance,x=Rating,color=Type),se=F)+
  geom_point(data=credit_pred,aes(y=Balance,x=Rating))+
  labs(title = "The model did not capture the pattern between Rating and Balance",
       x = "Rating",
       y = "Balance (USD)",color = "Label")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
## Do not modify this line!
m2<-lm(Balance~log_limit_per_card*Student+Rating,data=credit_new)
credit_pred<-add_predictions(add_residuals(credit_pred,m2),m2) %>%
  rename(pred2=pred,resid2=resid)
g6<-ggplot(credit_pred,aes(Balance,resid2))+
  geom_point()+
  geom_ref_line(h=0)+
  labs(title = "The residual still goes up as balance goes up",
       subtitle = "But the slope and the range residuals has decreased a lot",
       x = "Balance (USD)",y = "Residuals (USD)")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
g7<-ggplot(credit_pred)+
  geom_point(aes(Rating,Balance))+
  geom_smooth(aes(Rating,Balance,color="Data"),se=F)+
  geom_smooth(aes(Rating,pred1,color="Predictions 1"),se=F)+
  geom_smooth(aes(Rating,pred2,color="Predictions 2"),se=F)+
  labs(title = "The 2nd model did much better!",x = "Rating",
       y="Balance (USD)",color = "Label")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5),plot.subtitle=element_text(hjust = 0.5))
...
## Do not modify this line!
m3<-lm(Balance~log_limit_per_card*Student+Rating+Income+Limit+Cards,data=credit_new)

credit_pred<-add_predictions(add_residuals(credit_pred,m3),m3) %>%
...
#    African бн [78 б┴ 12] <lm>    16.3  1160    126     3    78        13 Male
#    African бн [78 б┴ 12] <lm>    44.5  3500    257     3    81        16 Female
#    African бн [78 б┴ 12] <lm>    33.0  3180    224     2    28        16 Male
#    African бн [78 б┴ 12] <lm>    35.0  3327    253     3    54        14 Female
#    African бн [78 б┴ 12] <lm>    50.7  3977    304     2    84        17 Female
#    African бн [78 б┴ 12] <lm>    54.7  4116    314     2    70         8 Female
#    African бн [78 б┴ 12] <lm>    58.1  4221    304     3    50         8 Male
#    African бн [78 б┴ 12] <lm>    29.7  3536    270     2    52        15 Female
#    African бн [78 б┴ 12] <lm>    19.1  3291    269     2    75        13 Female
#    African бн [78 б┴ 12] <lm>    26.1  3388    266     4    74        17 Female
#    бн with 300 more rows, and 6 more variables: Student <chr>, Married <fct>,
...
credit_model<-credit_new%>%
  group_by(Ethnicity)%>%
  nest()%>%
  mutate(model=data%>%map(~f1(.x)))%>%
  mutate(Data=map2(data,model,~add_residuals(.x,.y)))%>%
  unnest(Data)
...
#    Caucasian [158 б┴ 12] <lm>  [158 б┴ 13]     0.997         0.997  20.7     7955.
#    African бн  [78 б┴ 12] <lm>   [78 б┴ 13]     0.998         0.998  21.0     5023.
#    Asian      [74 б┴ 12] <lm>   [74 б┴ 13]     0.998         0.998  18.2     5721.
#    бн with 7 more variables: p.value <dbl>, df <int>, logLik <dbl>, AIC <dbl>,
#    BIC <dbl>, deviance <dbl>, df.residual <int>A tibble: 3 x 15
## Do not modify this line!
library(broom)

credit_model_glance<-credit_new%>%
  group_by(Ethnicity)%>%
  nest()%>%
  mutate(model=map(data,~f1(.x)))%>%
  mutate(resid=map2(data,model,~add_residuals(.x,.y)))%>%
