...
set.seed(0)
seed<-.Random.seed
x1_cauc<-rcauchy(n,0,1)
g_over_f1_cauc<-function(x){
  x>2
}
g_over_f1_cauc(x1_cauc)[1:5]
...
est2_cauc<-c(0.5-mean(g_over_f2_cauc(x2_cauc)),sqrt(var(g_over_f2_cauc(x2_cauc))/n))
...
## Do not modify this line!
est2_exp<-c(mean((g_over_f_exp(x_exp)+g_over_f_exp(1-x_exp))/2),
            sqrt(var((g_over_f_exp(x_exp)+g_over_f_exp(1-x_exp))/2)/n))
round(est2_exp, 4)
...
est3_exp<-c(mean(g_over_f_exp(x_exp)+c*(h(x_exp)+1/3)),sqrt(var(g_over_f_exp(x_exp)+c*(h(x_exp)+1/3))/n))
round(est3_exp, 4)
...
g_over_f_circ(x_circ, y_circ)[1:5]
...
g<-ggplot(circ,aes(x=x_circ,y=y_circ,color=selected))+
  geom_point(shape=20)+
  stat_function(fun=function(x) sqrt(1 - x^2),size=1.5,color='black')+
  stat_function(fun=function(x) -sqrt(1 - x^2),size=1.5,color='black')+
  coord_fixed()+
