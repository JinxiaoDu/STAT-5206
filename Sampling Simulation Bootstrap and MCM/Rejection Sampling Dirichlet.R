...
#    distribution. We want `f(x) б▄ M * g(x)` for `M > 1`.
...
#    To ensure for every x, `f(x) б▄ M * g(x)`, we just need to make sure 
...
## Do not modify this line!
f<-function(x){
  wxl<-c(x[1],x[2],1-x[1]-x[2])
  if(sum(x[1]+x[2]>1))
    return(0)
  return(ddirichlet(wxl,c(2, 2, 2)))
}
ycl<-function(x)
  -f(x)
M<--optim(c(0.3,0.3),ycl)[[2]]
g<-function(x){
...
sample_one<-function(){
  ...
  while(!accepted){
    ...
    x2=runif(1)
    u=runif(1)
    if(max(u,runif(1))<=f(c(x1,x2))/(M*g(c(x1,x2))))
      accepted=TRUE
    count=count+1
  }
  tibble(x1=x1,x2=x2,count=count)
}
set.seed(0)
sample_one()
...
sample_n<-function(n,seed=0) {
  set.seed(seed)
  reduce(map(1:n,~sample_one()),bind_rows)
}
...
acceptance_rate<-1/mean(samples$count)
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
sample_plot<-ggplot(samples,aes(x1,x2))+
...
sample_one2<-function(){
  ...
  while(!accepted){
    ...
    x2=rbeta(1,1.1,1.1)
    u=runif(1)
    if(max(u,runif(1))<=f(c(x1,x2))/(M2*g2(c(x1,x2))))
      accepted=TRUE
    count=count+1
  }
  tibble(x1=x1,x2=x2,count=count)
...
sample_n2<-function(n,seed=0) {
  set.seed(seed)
  reduce(map(1:n,~sample_one2()),bind_rows)
}
samples2<-sample_n2(100)
acceptance_rate2<-1/mean(samples2$count)