...
set.seed(100)
seed2<-.Random.seed
pvalue_result<-ks.test(df$x,pweibull,shape=3,scale=2)[[2]]
...
g1<-ggplot(df,aes(x=x))+
  ...
  stat_function(fun=partial(dweibull,shape=3,scale=2),aes(color="True value"))+
  scale_colour_manual(name="Legend",values=c("True value"="red"))+
  labs(title = "The density plot of random generated samples is similar to the true distribution",
       x = "Random generated x",y = "Density")+
  theme_light()+
