...
set.seed(0)
seed1<-.Random.seed
data<-tibble(x=rnorm(1000,0,1),y=map_dbl(x,~in_set(.x,1.5,2.5)),z=map_dbl(x,~dnorm(.x,0,1)))
cdf_estimate<-mean(data$y)
sd_estimate<-sd(data$y)
cdf_gold<-pnorm(2.5,0,1)-pnorm(1.5,0,1)
...
#    2 <dbl [10,000]> original <dbl [10,000]> <dbl [10,000]>                                                                                   3 <dbl [1,бн uniform(1.бн <dbl [1,бн <int [1,бн <dbl [1бн <dbl [1бн <dbl [1бн                                                                                                                                                                                 3 <dbl [1,000]> uniform(1.5, 2.бн <dbl [1,000бн <int [1,000бн <dbl [1,000бн <dbl [1,000бн <dbl [1,000бн
## Do not modify this line!
n<-1e4
set.seed(0)
seed2<-.Random.seed
...
## Do not modify this line!
result<-fit %>%
  transmute(mean=map_dbl(g_over_f,mean),se=map_dbl(g_over_f,sd)/sqrt(n),
...
## Do not modify this line!
variance_plot<-ggplot(result2,aes(y=mean,x=width))+
  geom_line()+
