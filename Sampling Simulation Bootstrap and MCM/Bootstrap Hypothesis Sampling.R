...
speed_hist<-ggplot(tibble(speed=speed),aes(speed))+
  ...
  labs(title="Histogram of speed",subtitle="There are outliers.",x="Speed",y="Count")+
  theme_light()
...
speed_qq<-ggplot(tibble(speed=speed),aes(sample=speed))+
  stat_qq()+
  stat_qq_line()+
  labs(title="QQ plot of speed",subtitle="The data is not normally distributed.")+
  theme_light()
...
boot_null<-function(x,mu0){
  function(){
    x_new<-x+mu0-mean(x)
    sample(x_new,length(x),replace=TRUE)
  }
}
mu0<-33.02
...
set.seed(0)
seed<-.Random.seed
bstrap<-tibble(b=c(1:1000),sample_null=map(b,~boot_speed_null()),muhat_null=map_dbl(sample_null,mean))
bstrap[1, 2][[1]]
...
bstrap_hist<-ggplot(bstrap,aes(muhat_null))+
  ...
  labs(title="Histogram of bootstrap samples",
       subtitle="The distribution is not normal.",x="Smaples",y="Count")+
  theme_light()
...
boot_pval<-function(muhat, mu0, muhat_null){
  wxl<-sum(muhat_null<mu0-abs(mu0-muhat))
  ycl<-sum(muhat_null>mu0+abs(mu0-muhat))
  (wxl+ycl)/length(muhat_null)
}
...
newspeed_qq<-ggplot(tibble(speed=newspeed),aes(sample=speed))+
  stat_qq()+
  stat_qq_line()+
  labs(title="QQ plot of newspeed",subtitle="The data is now normally distributed.")+
  theme_light()
...
set.seed(0)
seed<-.Random.seed
bstrap2<-tibble(b=c(1:1000),sample_null=map(b,~boot_speed_null2()),muhat_null=map_dbl(sample_null,mean))
