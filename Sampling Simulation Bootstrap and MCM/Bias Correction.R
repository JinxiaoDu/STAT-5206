...
#
## Do not modify this line!
boot_df<-function(df=df){
  sample_frac(df,replace=TRUE)
}
boot_ratio<-function(nboot,df=df){
  tibble(b=c(1:nboot),df_b=map(b,~boot_df(df)),theta_hat_b=map(df_b,ratio)) %>% pull(theta_hat_b) %>%unlist
}
...
set.seed(1)
seed1<-.Random.seed
results<-tibble(nboot=c(50, 100, 200, 500, 1000),theta_hat_b=map(nboot,~boot_ratio(.x,df)),
                bias=unlist(map(theta_hat_b,mean))-theta_hat,theta_hat_corrected=theta_hat-bias,type='normal') %>%
  select(-theta_hat_b)
...
rratio<-function(p,df){
  rmean(p,pull(df[,2]))/rmean(p,pull(df[,1]))
}
rmean(p=c(0.5, 0.5),c(y = c(7.06, 4.06), z = c(1.48, 6.18)))
...
#
## Do not modify this line!
boot_p_ratio<-function(df=df){
  n<-nrow(df)
  wxl<-sample(1:n,n,replace=TRUE)
  p<-tabulate(wxl)/n
  if(length(p)<n){
...
boot_ratio2<-function(nboot,df){
  tibble(b=c(1:nboot),df_b=map(b,~boot_p_ratio(df=df)))%>%
    unnest(df_b) %>%
    summarize(theta_hat_b=mean(theta),p_hat_b=list(mean_p(p)))
}
unlist(boot_ratio2(50,df)[,2])
set.seed(1)
boot_ratio2(3,tibble(y=c(7.06, 4.06),z=c(1.48,6.18)))
...
set.seed(1)
seed2<-.Random.seed
results2<-tibble(nboot=c(50, 100, 200, 500, 1000),theta_hat_p_hat=map(nboot,~boot_ratio2(.x,df))) %>%
  unnest(theta_hat_p_hat) %>%
  mutate(theta_hat=unlist(map(p_hat_b,~rratio(.x,df)))) %>%
  mutate(bias=theta_hat_b-theta_hat) %>%
  mutate(theta_hat_corrected=theta_hat-bias) %>%
  mutate(type='improved') %>%
  select(-theta_hat_b,-p_hat_b,-theta_hat)
...
## Do not modify this line!
lrh<-bind_rows(results,results2)
g1<-ggplot(lrh,aes(x=nboot,y=bias,color=type))+
  geom_line()+
  labs(title="The bias of improved method converge faster than that of the normal method",
       x="Nboot",y="Bias",color="Type")+
  theme_light()+
