...
set.seed(0)
seed<-.Random.seed
df<-tibble(u=runif(1000,0,1),x=Finv(u))
...
wxl<-df %>%
  filter(x>-50 & x<50)
g1<-ggplot(wxl,aes(x=x))+
  geom_histogram(aes(y=..density..),bins=100)+
  stat_function(fun=partial(dcauchy,location=0,scale=1),color = "red")+
...
g2<-ggplot(df,aes(sample=x))+
  geom_qq(distribution=qcauchy,dparams=list(0,1))+
  stat_qq_line(distribution=qcauchy,dparams=list(0,1),color='red')+
