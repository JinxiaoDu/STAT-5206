...
library(ggplot2)
g1<-ggplot(unif_pairs,aes(x=X1,y=X2))+
  geom_point()+
...
wxl<-function(x)
  dexp(x,rate=1.5)
g2<-ggplot(exp_rng,aes(x=E))+
  geom_histogram(aes(y=..density..),fill="white",color='black')+
  stat_function(fun=wxl,aes(color = "Theoretical"))+
...
wxl<-function(x)
  dnorm(x,0,1)
g3<-ggplot(gather(norm_rng,key='key',value='value'),aes(x=value))+
  geom_histogram(aes(y=..density..),fill="white",color='black')+
