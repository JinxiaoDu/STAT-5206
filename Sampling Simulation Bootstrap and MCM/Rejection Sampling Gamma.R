...
g(2)
mode<-optimize(f,lower=0,upper=10,maximum =TRUE)[[1]]
M_best<-f(mode)
...
#    # бн with 173 more rows
## Do not modify this line!
n<-1e3
result<-tibble(M=seq(M_best,5,0.025),Y=map(M,rejection_sampling,f,g),
               acceptance_rate=unlist(map(Y,length))/n)
...
## Do not modify this line!
acceptance_plot<-ggplot(result,aes(x=M,y=acceptance_rate))+
  geom_line()+
  labs(title="The acceptance rate decreases with M",
       x="M",y="Acceptance rate")+
  theme_light()
...
M2<-wxl(optimize(wxl,lower=0,upper=10,maximum=TRUE)[[1]])
set.seed(1)
seed<-.Random.seed
result2<-tibble(y=rnorm(n,mode,3),u=runif(n,0,1),likelihood_ratio=f(y)/(M2*g2(y)),
...
good_rejection_sampling_plot<-ggplot(result2,aes(x=y))+
  ...
  stat_function(fun=wxl,color='red',size=2)+
