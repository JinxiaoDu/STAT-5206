...
coeffs<-c(0,0,1,1)
m<-c(1,1,3,13)
wxl<-as.binary(c(2*coeffs[1]*m[4],2^2*coeffs[2]*m[3],2^3*coeffs[3]*m[2],2^4*coeffs[4]*m[1],m[1]))
m5<-reduce(wxl,xor)
m5<-as.numeric(m5)
...
next_in_seq<-function(coeffs,previous){
  ...
  wxl<-tibble(coeff_x_previous=coeff_x_previous,
              k=c(1:length(coeffs)),
              tmp=map2(coeff_x_previous,k,aux)) %>%
    pull(tmp)
  wxl<-as.binary(c(wxl,previous[length(coeffs)]))
  as.numeric(reduce(wxl,xor))
}
...
sobol<-function(coeffs,n,init){
  ...
  wxl<-length(coeffs)
  while(wxl<n){
    new_m<-next_in_seq(coeffs,previous[1:length(coeffs)])
    output<-c(output,new_m)
    previous<-output[length(output):1]
    wxl<-wxl+1
  }
  return(post_process(output))
}
...
#     9        3 uniform 0.901
#    10        4 sobol   0.812
#    # бн with 140 more rows
## Do not modify this line!
library(tidyr)
init<-c(1, 1, 3, 13)
set.seed(11)
seed<-.Random.seed
simul_tibble<-tibble(sequence=c(1:50),sobol=sobol(coeffs, 50, init),normal=rnorm(50,0.5,0.25),
                     uniform=runif(50,0,1)) %>%
  pivot_longer(c(2,3,4),names_to="type",values_to="value")
