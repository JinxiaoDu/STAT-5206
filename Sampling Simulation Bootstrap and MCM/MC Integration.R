...
set.seed(11)
seed<-.Random.seed
df<-tibble(s=runif(n,0,1),f_3=map_dbl(s,f_3)) 

ci_unif<-df %>%
  summarize(integral=mean(f_3),se=sqrt(var(f_3)/n),lower=integral-1.96*se,upper=integral+1.96*se)
...
ci_anti<-df2%>%
  summarise(integral=mean((f_3+f_3_anti)/2),se=sqrt(var((f_3+f_3_anti)/2)/n),
            lower=integral-1.96*se,upper=integral+1.96*se)
...
df3<-df2%>%
  mutate(f_2=f_2(s))
ci_control<-df3%>%
  summarize(c=-cov(f_3,f_2)/var(f_2),integral=mean(f_3+c*(f_2-integral_f_2)),
...
#     9        3 control     -0.0835
#    10        4 unif        -0.219
#    # бн with 29,987 more rows
## Do not modify this line!
library(tidyr)
cum_stats<-df3%>%
  mutate(cummean_f_3=cummean(f_3),cummean_f_3_anti=cummean(f_3_anti),cummean_f_2=cummean(f_2),
  ...
  filter(cum_c!='NaN')
errors<-cum_stats%>%
  mutate(sequence=c(1:(n-1)),
         unif=cummean_f_3-integral_f_3,
         antithetic=(cummean_f_3+cummean_f_3_anti)/2-integral_f_3,
         control=cummean_f_3-integral_f_3+cum_c*(cummean_f_2-integral_f_2))%>%
  select(sequence,unif,antithetic,control)%>%
  pivot_longer(cols=2:4,names_to = "sample_type",values_to = 'value')
