...
set.seed(0)
seed<-.Random.seed
result_boot<-tibble(b=c(1:B),permute=map(b,~tidy(model_mtcars(boot_permute_mtcars()))))
...
#    <int> <list>           <list>
#       1 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       2 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       3 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       4 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       5 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       6 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       7 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       8 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       9 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#      10 <tibble [2 ¡Á 5]> <tibble [2 ¡Á 5]>
#       ¡­ with 190 more rows
...
boot_parametric_factory<-function(df,model){
  ...
  function() {
    preds<-fitted(wxl)
    resids<-resid(wxl)
    df$mpg=preds+sample(resids,length(resids),replace=TRUE)
    df
  }
}
set.seed(0)
...
result_boot_tidy<-result_boot %>%
  pivot_longer(cols=-b,names_to='boot',values_to='value') %>%
  unnest(value) %>%
  mutate(full=ifelse(term=='wt',fit_mtcars[[1]][[2]],fit_mtcars[[1]][[1]]))
...
#
## Do not modify this line!
ci_basic_bootstrp<-function(thetab,thetahat,alpha=0.95){
  qb<-quantile(thetab,c((1-alpha)/2,(1+alpha)/2))
  tibble(lower=2*thetahat-qb[2],upper=2*thetahat-qb[1])
}
ci_percentile_bootstrp<-function(thetab,thetahat,alpha=0.95){
  qb<-quantile(thetab,c((1-alpha)/2,(1+alpha)/2))
  tibble(lower=qb[1],upper=qb[2])
}
ci_bootstrap<-result_boot_tidy%>%
  group_by(boot,term) %>%
  summarize(basic=list(ci_basic_bootstrp(estimate,mean(full))),
            percentile=list(ci_percentile_bootstrp(estimate,mean(full)))) %>%
  pivot_longer(cols=c("basic","percentile"),names_to="name",values_to="value") %>%
  unnest(value)
